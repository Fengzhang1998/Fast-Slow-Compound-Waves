...
dis_1_nN=cor_soma_x(1,nN)-cor_soma_x(1,1); %units: cm  需要先运行EF_coupling.m在工作区得到坐标矩阵cor_soma_x等等
...
cor_mea_x(2,3)=cor_soma_x(1,ceil(3*nN/4))-cor_soma_x(1,1);  %　三个电极分别放在第50、100、150这三列细胞的位置处

wg_matrix_s=zeros(n_y_mea*n_x_mea,nN);  % 列数为nN，分别代表每一列细胞对该虚拟电极产生的胞外电势
...
t0=0000;
Is_d1_sum=gc_s_d1*wg_matrix_d1*(Vms-Vmd1); % 列和行一一对应
